...
CLI tool that derives screen-protector overlay images from a device photo. Full spec: `screenshield/PROMPT.md`. Build proceeds in stages, stopping after each for review (see PROMPT.md "Build order"). **Stages 1-5 are settled** as of this writing (body detection, compositing/opacity/canvas framing, cutouts, `--target screen`/`recess`, SVG export); stage 6 (batch mode) has not started.
