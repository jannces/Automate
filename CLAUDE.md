...
**Stroke = 1pt CMYK 54.6/46.1/45.7/11.1, butt cap, miter join** → Illustrator's RGB conversion is `#787878`, at 1px on a 1500px canvas. The previous constants (1.317px, `#979797`) were solved by assuming the brightest-possible stroke peak was saturated; that gave the same total ink (≈137) and so could not be told apart on straight edges. The references' diagonal chamfers contain pixels at 116-125, impossible for a `#979797` stroke, so those constants are refuted. Straight edges in `ref_1x` total 142-143 of ink; Illustrator's own 1px export totals 135; the renderer totals ~141 (known +0.045px supersample residual in `stroke_coverage_map`).
